...
INFO_STR = r"Geant4  |  Bar Strip Detector  |  70 cm Fe + BC404  |  FTFP\_BERT  |  $B_x = 0.1\,\mathrm{T}$"
...
def plot_detector_layout(out_dir, seed=42):
    ...
    fig.suptitle(
        r"Bar Strip Detector + $B_x = 0.1\ \mathrm{T}$ — Configuración del sistema" + "\n"
        r"Absorbedor: G4\_Fe  70$\times$70$\times$70 cm  |  "
        r"Centellador: BC404  |  "
        r"Campo magnético: $B_x = 0.1\ \mathrm{T}$ en región vacío (fuente → Fe)",
    ...
    # ── Región del campo magnético Bx = 0.1 T (z = -200 cm a z = 0) ──
    ...
    legend_elems = [
        Patch(facecolor="deepskyblue", alpha=0.25,
              label=r"Región $B_x = 0.1\ \mathrm{T}$"),
