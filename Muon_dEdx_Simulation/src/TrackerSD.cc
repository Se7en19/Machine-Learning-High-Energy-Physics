...
G4bool TrackerSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
  ...
  // Momemnto en el PreStepPoint (inicio del paso) 
  newHit->SetMomentum(step->GetPreStepPoint()->GetMomentum());
  // carga de la particula (en unidades de la carga del electron )
  newHit->SetCharge(step->GetTrack()->GetDefinition()->GetPDGCharge());
