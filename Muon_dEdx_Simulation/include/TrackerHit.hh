...
class TrackerHit : public G4VHit
{
  public:
    ...
    void SetCharge (G4double q) { fCharge = q; }
    ...
    // dE/dx 
    // regresa Mev/mm o regresa 0 si el paso tiene longitud nula 
    G4double GetDEDX() const{
      return (fStepLength>0.) ? fEdep/fStepLength : 0.;
    }

    // p/q 
    // Regresa GeV/c. Convenio: signo del momento en Z*|p|/|q| 
    G4double GetRigidity() const{
      if (fCharge==0.) return 0.;
      return fMomentum.mag()/fCharge;
    }
  private:
    G4int fTrackID = -1;
    G4int fChamberNb = -1;
